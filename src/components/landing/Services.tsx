      ...
      {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          {services?.map(service => <Card key={service.id} className="border transition-all duration-300 hover:border-accent hover:shadow-lg hover:shadow-accent/10 rounded-2xl flex flex-col h-full">
              <CardContent className="pt-6 space-y-4 flex-1 flex flex-col">
                {service.service_icon?.icon_url ? <img src={service.service_icon.icon_url} alt={service.service_icon.name} className="h-12 w-12 object-contain" /> : service.service_icon?.name ? <div>{getIcon(service.service_icon.name)}</div> : <div className="h-12 w-12 bg-accent/10 rounded-lg flex items-center justify-center">
                    <span className="text-2xl">🔧</span>
                  </div>}
                <div className="my-[40px]">
                  <h3 className="text-xl font-semibold mb-2">{service.title}</h3>
                  {/* Tools subtitle - stored in description field temporarily until we add a tools column */}
                  {service.description.includes("Tools:") && <p className="text-sm text-accent/80 mb-3">
                      {service.description.split("\n")[0]}
                    </p>}
                  <p className="text-muted-foreground leading-relaxed text-sm my-[40px]">
