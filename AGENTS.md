...
- Project ref: `tbdhzxarsshzoweyndha`
- Local config: `supabase/config.toml`
- MCP server added for this project: `https://mcp.supabase.com/mcp?project_ref=tbdhzxarsshzoweyndha`

Important implementation detail:

- `.env` and `.env.example` define `VITE_SUPABASE_*` values.
- `src/integrations/supabase/client.ts` currently hardcodes the Supabase URL and anon key instead of reading `import.meta.env`.
- Treat the hardcoded client and `.env.example` as inconsistent until unified.
